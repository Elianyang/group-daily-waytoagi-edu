---
name: group-daily-waytoagi-edu
description: |
  群日报技能（WaytoAGI-EDU 专属版）。把 WaytoAGI-EDU 群或指定微信群一天（或指定时间段）的聊天记录，做成带紫金魔法小鹿吉祥物、群配色和固定署名的日报
...
# Group Daily（WaytoAGI-EDU）· 群日报


## WaytoAGI-EDU 专属品牌规范（必须延续）

本技能是 `group-daily（WaytoAGI-EDU）` 专属版。生成 WaytoAGI-EDU 群日报时必须延续以下风格：

- **群身份**：WaytoAGI-EDU，一个教育 AI 共创社群。
- **视觉基调**：紫色魔法、金色星星、暖米白纸感、教育陪伴感；避免普通商务蓝白风。
- **吉祥物 / Logo**：使用 `assets/waytoagi-edu/mascot_logo.png` 作为报头 logo，`mascot_hero.png` 作为隐约背景装饰。
- **表情包资产**：`assets/waytoagi-edu/sticker_01.png` 到 `sticker_16.png` 来自用户提供的小鹿表情包合集，可在日报中按当天情绪轮换；story 可通过 `brand_stickers` 指定贴纸 id。
- **固定署名**：每一份日报底部必须有隐约字样：`Co-Created with 进击中的丫丫老师`。
- **配色**：深紫 `#2B104C`、魔法紫 `#6E2EB8`、星星金 `#FFD84D`、暖米白 `#FFF8E9`、教育蓝 `#39B7FF`、腮红粉 `#FF8FA3`。
- **纹理禁忌**：不要使用明显斜杠/斜向条纹背景；优先用柔和星点、圆点、纸感渐变和魔法光晕。
- **字体气质**：不要用过于古板的严肃衬线/等宽大字距；标题、副标题、品牌小字优先使用手绘感、可爱感字体，如 `HanziPen SC`、`Hannotate SC`、`Klee`、`Chalkboard SE`、`Marker Felt` 等系统字体 fallback。
- **大小写硬约束**：品牌必须写作 `WaytoAGI-EDU`。禁止 `WAYTOAGI-EDU`、`WaytoAGI`、`W A Y T O A G I - E D U` 等错误形式，也不要为了装饰把品牌字母拆开。
- **默认群名**：当用户说“我们群”“EDU 群”“WaytoAGI-EDU 群”时，默认指 `WaytoAGI-EDU 情报局`。
